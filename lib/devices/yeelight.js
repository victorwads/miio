	...
	.with(Light, Fading, MiioApi, Power, Dimmable, Nameable)
{
	...
	async changeBrightness(brightness, options) {
		if(brightness <= 0) {
			return this.changePower(false);
		} else {
			let promise;
			if(options.powerOn && await this.power() === false) {
			...
			return promise.then(() =>
				this.call(
					'set_bright',
					Yeelight.withEffect(parseInt(brightness), options.duration), {
					refresh: [ 'brightness' ]
				})
			).then(MiioApi.checkOk);
		}
	...
	.with(MiioApi, Colorable, ColorTemperature, ColorFull)
{
	...
	async changeColor(color, options) {
		if(color.is('temperature')) {
			// The user has request a color via temperature

			const range = await this.colorTemperatureRange();
			const temperature = Math.min(Math.max(color.temperature.kelvins, range.min), range.max);
